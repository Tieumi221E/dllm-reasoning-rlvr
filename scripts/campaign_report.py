...
RECIPES = {
    "d14_t12": (
        "results/full36_d14t12_1000.json",
        lambda d, t: d <= 4 and t <= 2,
    ),  # pretrain/SFT region
    "d12_t12": (
        "results/full36_d12t12_1000.json",
        lambda d, t: d <= 2 and t <= 2,
    ),  # shallow-simple
    "d34_t12": (
        "results/full36_d34t12_1000.json",
        lambda d, t: 3 <= d <= 4 and t <= 2,
    ),  # mid-depth simple
    "d56_t12": (
        "results/full36_d56t12_1000.json",
        lambda d, t: 5 <= d <= 6 and t <= 2,
    ),  # deep simple
    "d12_t34": (
        "results/full36_d12t34_1000.json",
        lambda d, t: d <= 2 and 3 <= t <= 4,
    ),  # shallow mid-complex
    "d12_t56": (
        "results/full36_d12t56_1000.json",
        lambda d, t: d <= 2 and 5 <= t <= 6,
    ),  # shallow high-complex
    "d34_t34": (
        "results/full36_d34t34_1000.json",
        lambda d, t: 3 <= d <= 4 and 3 <= t <= 4,
    ),  # mid diagonal
    "d16_t12": (
        "results/full36_d16t12_1000.json",
        lambda d, t: d <= 6 and t <= 2,
    ),  # depth-axis broad
    "d14_t16": (
        "results/full36_d14t16_1000.json",
        lambda d, t: d <= 4 and t <= 6,
    ),  # complexity-axis broad
    "d16_t16": (
        "results/full36_d16t16_1000.json",
        lambda d, t: d <= 6 and t <= 6,
    ),  # full coverage
}
...
def load(path):
    ...
    c = defaultdict(list)
    for x in d:
        c[(x["depth"], x["tier"])].append(
            (x["pass_k"]["1"], x["pass_k"].get("128", 0.0))
        )
    return c
        ...
        for j in range(6):
            if not np.isnan(g[i, j]):
                col = (
                    "black"
                    if diverging
                    else ("white" if g[i, j] < vmax * 0.6 else "black")
                )
                ax.text(
                    j,
                    i,
...
def fig_base(base):
    ...
    ax = axes[2]
    ax.plot(
        list(DEPTHS), dep, "-o", color="#d62728", label="vs depth (mean over tiers)"
    )
    ...
    fig.tight_layout()
    fig.savefig(f"{OUT}/fig_base_capability.png", dpi=130)
    plt.close(fig)
...
def fig_rl_grid(base, rls):
    ...
    for row, (name, (rl, trained)) in enumerate(rls.items()):
        for col, (metric, idx) in enumerate(
            [("SG = Δpass@1", 0), ("CG = Δpass@128", 1)]
        ):
            ax = axes[row, col]
            g = np.full((6, 6), np.nan)
            for dd, tt in base:
    ...
    fig.suptitle(
        "RLVR effect per recipe - SG (sharpening) vs CG (ceiling gain)\n"
        "green box = RL training region; red = gain, blue = loss",
        fontweight="bold",
    )
    fig.tight_layout(rect=[0, 0, 1, 0.99])
    fig.savefig(f"{OUT}/fig_rl_grid.png", dpi=120)
    plt.close(fig)
...
def fig_best_recipe(base, rls):
        ...
        for dd in DEPTHS:
            for tt in TIERS:
                ...
                if best_name is not None and best_m > 0 and best_t >= T_GATE:
                    ...
                    win_lab[dd - 1][tt - 1] = "-"
        im = ax.imshow(
            win_val, origin="lower", cmap="YlOrRd", vmin=0, vmax=0.25, aspect="auto"
        )
        ax.set_xticks(range(6))
        ax.set_xticklabels([f"T{t}" for t in TIERS])
        ax.set_yticks(range(6))
    ...
    fig.suptitle(
        "Allocation map: which training region wins each test cell "
        "(paired, noise-gated)",
        fontweight="bold",
    )
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(f"{OUT}/fig_best_recipe.png", dpi=130)
    plt.close(fig)
...
def _summ(sg, cg):
    ...
    if tsg < 2 and tcg < 2:
        ...
        lab = "mixed"
    return (
        f"SG {msg:+.3f}(t{tsg:+.1f})  CG {mcg:+.3f}(t{tcg:+.1f})  "
        f"SG-CG {mct:+.3f}(t{tct:+.1f})  -> {lab}"
    ), len(sg)
...
def tables(base, rls):
    ...
    g1, g128 = grid(base, 0), grid(base, 1)
    p("=" * 78)
    p("BASE (SFT-1800)  pass@1 / pass@128   [D1-6 x T1-6]")
    p("=" * 78)
    ...
    p(
        "  vs depth: "
        + "  ".join(f"D{d}={np.nanmean(g128[d - 1, :]):.2f}" for d in DEPTHS)
    )
    p(
        "  vs tier : "
        + "  ".join(f"T{t}={np.nanmean(g128[:, t - 1]):.2f}" for t in TIERS)
    )

    # ---- sharpening vs capability, per recipe x region (trained / out-of-region) ----
    p("\n" + "=" * 78)
    p(
        "SHARPENING vs CAPABILITY  (paired; contrast = SG-CG: >0 sharpening, <0 capability)"
    )
    p("=" * 78)
    for name, (rl, trained) in rls.items():
        ...
        for dd, tt in base:
            if (dd, tt) not in rl:
                continue
            sg = list(paired_diff(base, rl, dd, tt, 0))
            cg = list(paired_diff(base, rl, dd, tt, 1))
    ...
    p("\n" + "=" * 78)
    p(
        "OFF-DIAGONAL TRANSFER  (CG: in-region vs out-region; peak-CG cell & its location)"
    )
    p("=" * 78)
    ...
    for name, (rl, trained) in rls.items():
        ...
        p(
            f"   {name:>8}  {mi:+.3f}(t{ti:+.1f})  {mo:+.3f}(t{to:+.1f})  "
    ...
    p(
        "   >0.5 means most capability gain lands OUTSIDE the trained region (= transfer)."
    )
    out.close()
...
if __name__ == "__main__":
    ...
    tables(base, rls)
    print(
        f"wrote {OUT}/fig_base_capability.png, fig_rl_grid.png, fig_best_recipe.png, campaign_tables.txt"
    )
