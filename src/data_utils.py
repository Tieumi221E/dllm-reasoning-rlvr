...
RECIPE_FILTERS: Dict[str, Callable[[int, int], bool]] = {
    ...
    "d2_3": lambda d, t: 2 <= d <= 3
    and t <= 2,  # highest reachable-headroom region (pass@128 ~0.83)
    "d4_5": lambda d, t: 4 <= d <= 5 and t <= 2,  # competence-boundary region
    # Exact cell definitions aligned with 2605.26934 Table 16:
    "depth_mid": lambda d, t: 5 <= d <= 7
    and t <= 2,  # Depth-Mid: D5-D7×T1-T2 (6 cells) - single depth axis
    "depth_high": lambda d, t: 8 <= d <= 10
    and t <= 2,  # Depth-High: D8-D10×T1-T2 (6 cells) - deep region, direct
    "depth_uniform": lambda d, t: t
    <= 2,  # Depth-Uniform: D1-D10×T1-T2 (20 cells) - full depth axis
    "depth_5_10": lambda d, t: 5 <= d <= 10
    and t <= 2,  # D5-D10×T1-T2 (12 cells) - whole OOD depth band
    "offbase_mix": lambda d, t: not (
        d <= 4 and t <= 2
    ),  # Offbase-Mix: everything outside the pretrain block (52 cells) - joint coverage
    ...
    "d14_t16": lambda d, t: d <= 4
    and t <= 6,  # complexity coverage (in-depth, all complexity)
...
def expand_graph_to_tasks(item: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    meta = {
        ...
        "complexity": (
            item["complexity"]["composite_score"]
...
def prepare_bin(jsonl_path, bin_prefix, tokenizer, max_length=2048):
    ...
    with open(jsonl_path, "r", encoding="utf-8") as fin, open(tmp_bin, "wb") as fbin:
    ...
    print(
        f"Binary ready: {len(offsets):,} samples, {pos:,} tokens → {bin_path}",
        flush=True,
    )
    return bin_path, offsets, lengths
    ...
    with open(tmp_path, "w", encoding="utf-8") as fout:
        for graph in _iter_raw_json_array(rl_json_path):
            ...
            for task in tasks_for_graph:
                ...
                count += 1
            if max_per_cell is not None:
                cell_counts[cell_key] = cell_counts.get(cell_key, 0) + len(
                    tasks_for_graph
                )
...
class KGReasoningDataset(Dataset):
    ...
    def __init__(self, file_path, tokenizer, max_length=2048, mode="pretrain"):
        ...
        if mode == "pretrain":
            ...
            print(
                f"Eval dataset: {len(self.tasks):,} tasks from {len(graphs):,} graphs"
            )
