...
def make_batch(samples, tok, block_length, device, eps=1e-3):
    ...
    def to_tensor(values, dtype=torch.long):
    ...
    return (
        ...
        to_tensor(target),
        to_tensor(attn),
        to_tensor(loss, torch.bool),
        to_tensor(pmask, torch.float),
    )
...
def train(args):
    ...
    print(
        f"tokenizer vocab={tok.vocab_size} EOS={tok.eos_token_id} MASK={tok.mask_token_id}"
    )
    model = (
    ...
    print(
        f"model {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M  block={args.block_length} (block-wise semi-AR)"
    )

    optimizer = AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
    scheduler = get_cosine_schedule_with_warmup(
        optimizer, int(0.05 * args.max_steps), args.max_steps
    )
    ...
    for step in range(1, args.max_steps + 1):
        ...
        while len(samples) < args.batch_size:
            ...
            if ex:
                samples.append(ex)
        body, target, attn, lm, pmask = make_batch(
            samples, tok, args.block_length, device
        )
        ...
        if step % 20 == 0:
            print(
                f"step={step}/{args.max_steps} loss={running / 20:.4f} lr={scheduler.get_last_lr()[0]:.2e}"
            )
            running = 0.0
        if step % args.save_every == 0:
            ck = os.path.join(args.out_dir, f"step_{step}")
            model.save_pretrained(ck, safe_serialization=False)
...
def main():
    ...
    p.add_argument(
        "--model_path",
        required=True,
        help="Pretrained backbone checkpoint dir (see README Attribution)",
    )
