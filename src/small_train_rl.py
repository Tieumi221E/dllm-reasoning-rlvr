...
Usage:
  CUDA_VISIBLE_DEVICES=0 python -m src.small_train_rl \
...
def train(args):
    ...
    model = DiffusionTransformerLM.from_pretrained(
        args.sft_path, torch_dtype=torch.bfloat16
    ).to(device)
    print(
        f"model {sum(p.numel() for p in model.parameters()) / 1e6:.1f}M (full FT)  MASK={mask_id} EOS={eos_id}"
    )
    ...
    print(
        f"TraceRL/GRPO: steps={args.total_steps} G={args.G} B={args.B} lr={args.lr} "
    ...
    for opt_step in range(1, args.total_steps + 1):
        ...
        while len(rollout_data) < args.B and draws < args.max_draws:
            ...
            pid = tok.encode(
                f"Story: {task['story']} Question: {task['question']} Answer:"
            )[:-1]
            ...
            for r in gen:
                ...
                sm = [i // args.block_length for i in range(nreal)] + [-1] * (
                    args.gen_length - nreal
                )
                g_rows.append(r)
                m_rows.append(sm)
            ...
            rollout_data.append(
                dict(
                    ...
                    rewards=rewards,
                    rewards_mp=rewards_mp,
                    rewards_ma=rewards_ma,
                )
            )
        torch.cuda.empty_cache()
        win_draws += draws
        win_accepted += len(rollout_data)
        if not rollout_data:
            print(
                f"step={opt_step}/{args.total_steps} no-variance after {draws} draws - skip"
            )
            continue
        ...
        if opt_step % args.log_every == 0:
            ...
            ns = max(n_ppo, 1)
            print(
                f"step={opt_step}/{args.total_steps} policy={tp / ns:.4f} kl={tk / ns:.4f} "
