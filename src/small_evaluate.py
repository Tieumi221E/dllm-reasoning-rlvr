...
Usage:
  CUDA_VISIBLE_DEVICES=1 python -m src.small_evaluate \
...
CORRUPT = re.compile(
    r"brokenact|intactbrok|brokenbroken|actintact|"
    r"\b(\w+)\s+\1\s+\1\b"
)
...
def main():
    ...
    ap.add_argument(
        "--val_data",
        default="data_generation/output_stratified_pretrain_3200000_0.1562_rl1x_1200000_eval_4800_0.3722_20260527_193136/eval.json",
    )
    ap.add_argument(
        "--task_cache",
        default=None,
    ...
    model = (
        DiffusionTransformerLM.from_pretrained(
    ...
    print(
        f"model {args.model_path}  MASK={mask_id} EOS={eos_id}  block-gen "
    ...
    if args.task_cache:
        ...
        all_tasks = (
            t for g in json.load(open(args.val_data)) for t in expand_graph_to_tasks(g)
        )
    per_cell = {}
    if True:
        for t in all_tasks:
            ...
            if args.min_depth and d < args.min_depth:
                continue
            if args.max_depth and d > args.max_depth:
                continue
            if args.min_tier and tier < args.min_tier:
                continue
            if args.max_tier and tier > args.max_tier:
                continue
    ...
    for i, t in enumerate(tasks):
        prompt_ids = tok.encode(
            f"Story: {t['story']} Question: {t['question']} Answer:"
        )[:-1]
        texts = []
        ...
        results.append(
            dict(
                ...
                rstd=st.pstdev(rs),
                pass_k={
                    str(k): _passk(nc, args.num_samples, k)
        ...
        print(
            f"  task{i} D{d}T{tier} strict={nc}/{args.num_samples} "
    ...
    json.dump(
        dump, open(args.out_path + ".samples.json", "w"), indent=2, ensure_ascii=False
    )
    ...
    for r in results:
        ...
        pk = r["pass_k"]
        c["p1"].append(pk["1"])
        c["p8"].append(pk.get("8", pk.get("4", pk["1"])))
        c["ma"].append(r["m_A"])
        c["mp"].append(r["m_P"])
        c["rstd"].append(r["rstd"])
    print(
    ...
    for k in sorted(cells):
        c = cells[k]
        p1, p8 = mean(c["p1"]), mean(c["p8"])
        allp1.append(p1)
        allp8.append(p8)
        allg.append(p8 - p1)
        print(
            f"D{k[0]}T{k[1]:>2} | {p1:5.2f} {p8:5.2f} {p8 - p1:+6.2f} | "
            f"{mean(c['ma']):5.2f} {mean(c['mp']):5.2f} {mean(c['rstd']):5.2f}"
        )
    print("-" * 52)
    print(f"{'MEAN':>8} | {mean(allp1):5.2f} {mean(allp8):5.2f} {mean(allg):+6.2f} |")
